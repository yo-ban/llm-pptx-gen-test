...
セクションのテキスト量が多くなる場合には、header_font_sizeおよびfont_sizeの値を調整してください。
目安として、デフォルトのフォントサイズでは、11行前後でスライドの枠からはみ出すことが多いです。

また、画像が必要なスライドの場合、画像検索ツールを使用して先に画像のパスを取得することが推奨されます。
...
- **`content_with_image_right` や `content_with_table_right` は、付随テキストが少ない場合にのみ使用してください。** 長文の箇条書きには適していません。
